...
| Preview | `POST /preview/leads/` | **$0.002** | per returned (masked) result — free once your account passes $2,000 total spend |
| Phone Finder | `POST /phone/find/` (+ `/bulk/`) | **$0.40** | per **found** phone |

<Info>Prices above are the Tier 0 (standard) rate. The more you spend over the lifetime of your account, the higher your discount tier and the lower most prices become — automatically. See [Spend & Save tiers](/billing/managing-billing).</Info>

<Note>**Email Validation is excluded from volume discounts** — it stays at $0.005 per email on every tier. Everything else in the table above scales down with your tier.</Note>

## How a charge is calculated

```
amount charged (USD) = billable results × operation price × your tier multiplier
# (tier multiplier is 1.0 for Email Validation — see the note above)
```
...
- **Preview count** (`POST /preview/leads/count/`) — sizing a preview audience is free.
...
- **Preview** returns real (masked) leads at $0.002/result — and becomes free once your account passes $2,000 total spend.
