...
Preview API returns masked cached lead data, such as name, title, company, location, industry, and seniority, with a unique lead ID. Browsing costs \$0.002 per lead shown — a tenth of the enrichment price — and the full price is only paid when a user reveals a profile or contact data.
...
5. Pay \$0.002 per lead shown in step 2, and the enrichment price only for the ones selected
