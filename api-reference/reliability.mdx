...
We publish the targets we hold ourselves to, and the live measurement against each one, on [status.generect.com](https://status.generect.com) — sampled every minute over the last 24 hours:

| Metric | Target |
| --- | --- |
| API latency (p95) | ≤ 1.5s |
| Error rate (5xx) | < 0.10% |
| Email Finder hit rate | ≥ 88% |

These are **operational targets**, published so you can see the same numbers we do — not a contractual guarantee. Latency targets apply to database-mode operations; real-time operations depend on live upstream sources (see [recommended timeouts](/api-reference/limits)).

Component-level status and **90-day daily uptime history** are on the same page, and you can subscribe there for incident notifications.

A contractual SLA with agreed uptime and support-response commitments is available on Enterprise agreements — [contact sales](https://calendly.com/andrii-kolpakov/_sales_api_insight).
